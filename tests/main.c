...
#ifdef DEBUG
#define LOG(fmt, arg...) fprintf(stdout, "[gst_test] %s:%d, " fmt, __FUNCTION__, __LINE__, ##arg);
#else
#define LOG(fmt, arg...)
#endif
...
int previous_play_status = PLAY_STATUS_NORMAL;
...
int parse_file(const char *file, int32_t *has_audio,
               int32_t *has_video)
{
    if (file == NULL || has_audio == NULL || has_video == NULL)
    {
        LOG("bad parameter!\n");
        return ERROR_CODE_BAD_PARAMETER;
    }
    ...
    discoverer_create(&hdl);
    discoverer_run(hdl, uri);
    *has_video = discoverer_has_video(hdl);
    *has_audio = discoverer_has_audio(hdl);
    discoverer_destroy(hdl);
...
int create_pipeline(const char *file, GstElement **pipeline)
{
    ...
    if ((-1 == has_audio) || (-1 == has_video))
    {
        LOG("parse_file failed， has_audio:%d, has_video:%d!\n", has_audio, has_video);
        return ERROR_CODE_BASE_ERROR;
    }
...
int pause_(GstElement *pipeline)
{
    GstStateChangeReturn ret;
...
int wait_for_start()
{
    return wait_for_status(PLAY_STATUS_STARTED | PLAY_STATUS_ERROR | PLAY_STATUS_FINISHED);
}

int wait_for_finish()
{
    return wait_for_status(PLAY_STATUS_ERROR | PLAY_STATUS_FINISHED);
}
               ...
               int32_t *check_error_is_enabled)
{
    ...
    if (media_path == NULL || random_the_cmd == NULL || random_the_file == NULL || sleep_is_enabled == NULL || check_error_is_enabled == NULL)
    {
        LOG("bad parameter!\n");
        return ERROR_CODE_BAD_PARAMETER;
    }
                      ...
                      int32_t filter_len)
{
    ...
    if (path == NULL || filter == NULL || filter_len < 1)
    {
        return ERROR_CODE_OK;
    }
                     ...
                     int32_t filter_len, char ***files, int32_t *num)
{
    ...
    while (dir_cache_index >= 1)
    {
        ...
        if (dir == NULL)
        {
            ...
            {
                if (path_was_filtered(curr_path, filter, filter_len) == ERROR_CODE_OK)
                {
        ...
        while ((entry = readdir(dir)) != NULL)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        sprintf(path_cache, "%s/%s", curr_path, entry->d_name);
                        if (path_was_filtered(path_cache, filter, filter_len) == ERROR_CODE_OK)
                        {
