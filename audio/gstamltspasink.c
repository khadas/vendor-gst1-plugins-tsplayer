...
static gboolean gst_amltspasink_propose_allocation (GstBaseSink * sink,
  GstQuery * query);
...
static GstFlowReturn gst_amltspasink_wait_event (GstBaseSink * sink,
  GstEvent * event);
static GstFlowReturn gst_amltspasink_prepare (GstBaseSink * sink,
  GstBuffer * buffer);
static GstFlowReturn gst_amltspasink_prepare_list (GstBaseSink * sink,
  GstBufferList * buffer_list);
static GstFlowReturn gst_amltspasink_preroll (GstBaseSink * sink,
  GstBuffer * buffer);
static GstFlowReturn gst_amltspasink_render (GstBaseSink * sink,
  GstBuffer * buffer);
...
{
  GST_DEBUG_OBJECT (amltspasink, "init");
...
void
gst_amltspasink_set_property (GObject * object, guint property_id,
  const GValue * value, GParamSpec * pspec)
{
  ...
  switch (property_id) {
    case PROP_VOLUME:
    {
      ...
      GST_DEBUG_OBJECT (amltspasink, "set_property, volume: %d", volume);
...
void
gst_amltspasink_get_property (GObject * object, guint property_id,
...
void
gst_amltspasink_dispose (GObject * object)
{
  GstAmltspasink *amltspasink = GST_AMLTSPASINK (object);
...
void
gst_amltspasink_finalize (GObject * object)
{
  GstAmltspasink *amltspasink = GST_AMLTSPASINK (object);
...
gst_amltspasink_change_state (GstElement * element, GstStateChange transition)
{
  ...
  ret = GST_ELEMENT_CLASS (gst_amltspasink_parent_class)->change_state (
    element, transition);
  ...
  GstClockTime * start, GstClockTime * end)
{
  ...
  *end = GST_CLOCK_TIME_NONE;
  ...
  ret = gst_get_timestamp_of_gstbuffer(sink, buffer, start);
  if (ret == TRUE) {
...
gst_amltspasink_query (GstBaseSink * sink, GstQuery * query)
{
  ...
  GST_DEBUG_OBJECT (amltspasink, "query--%s", GST_QUERY_TYPE_NAME(query));
...
gst_amltspasink_event (GstBaseSink * sink, GstEvent * event)
{
  ...
  GST_DEBUG_OBJECT (amltspasink, "event--%s", GST_EVENT_TYPE_NAME(event));
  ...
  switch (GST_EVENT_TYPE (event)) {
    ...
    case GST_EVENT_SEGMENT:
    {
      ...
      GST_DEBUG_OBJECT (amltspasink, "rate--%f", segment.rate);

      if (amltspasink->priv.rate != segment.rate) {
        set_audio_rate(segment.rate);
        amltspasink->priv.rate = segment.rate;
      }
