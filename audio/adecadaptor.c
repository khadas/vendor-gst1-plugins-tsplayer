...
static uint64_t timeout_ms = 3000;
static uint32_t sleep_ms = 500;

#ifdef DEBUG
#define LOG(fmt, arg...) fprintf(stdout, "[adecadaptor] %s:%d, " \
...
static
am_tsplayer_audio_codec codec_char_to_enum(const char *codec)
{
    if (codec == NULL) {
...
int set_audio_rate(double rate)
{
    ...
    // TODO: uncomment this.
    // ret = AmTsPlayer_setPlaybackRate(session, rate);
    if (ret != AM_TSPLAYER_OK) {
        pthread_mutex_unlock(&lock);
        LOG("AmTsPlayer_setPlaybackRate failed: %d\n", ret);
        return ERROR_CODE_BASE_ERROR;
    }
...
int flush_adec()
{
    ...
    // TODO: uncomment this.
    // ret = AmTsPlayer_flushAudio(session);
    if (ret != AM_TSPLAYER_OK) {
        pthread_mutex_unlock(&lock);
        LOG("AmTsPlayer_flushAudio failed: %d\n", ret);
        return ERROR_CODE_BASE_ERROR;
    }
